...
class EvaluationExpert(GenericAPIView):
    ...
    queryset = evModels.objects.all()
    def get(self, request):
        quaryset = evModels.objects.all()
        output = Evaluation_serializer(quaryset, many=True)
        return Response(output.data)

# THE DATA MUST POST TO IT IS email, gender, username, name, password
...
class InsurserView(GenericAPIView):
    ...
    def get(self, requet):
        ...
        return Response(s.data, status=status.HTTP_200_OK)
# THE DATA MUST POST TO IT IS email, gender, username, name, password, insurer_start and insurer_end
...
# THE DATA MUST POST TO IT IS name, address, phone_number
    def post(self, request):
        data = JSONParser().parse(request)
...
# THE DATA MUST POST TO IT IS companyname, location,  evalution_time, email
    def post(self, request):
        data = JSONParser().parse(request)
        c = Company.objects.get(name=data['name'])
...
# THE DATA MUST POST TO IT IS name, parent
...
# THE DATA MUST POST TO IT IS username, name
    def post(self, request):
        data = JSONParser().parse(request)
